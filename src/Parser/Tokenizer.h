...
private:
  ...
  int  get();
  bool getChar()  throw(std::runtime_error);
  ...
  bool aSingleQuote;
  bool isBeginQuote(bool isPlainText);
  bool isBeginQuote();
  bool isEndQuote();
  std::string eatQuote();
...
inline std::string Toker::getTok() { return Toker::getTok(false); }

inline bool Toker::isBeginQuote() { return Toker::isBeginQuote(false); }
