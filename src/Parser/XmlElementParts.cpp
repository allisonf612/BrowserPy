...
//  Application:   ProtopartType for CSE687 Pr1, Sp09                  //
...
bool XmlParts::get()
{
  ...
  do
  {
    ...
    /*
     TODO: this would be changed for XML if there
     * was handling for escape characters in plaintext
     */
    if(toks.size() == 0)
    {
        if(tok == "<")
...
std::string& XmlParts::operator[](int n)
{
  if(n < 0 || toks.size() <= (size_t)n)
    throw "XmlParts index out of range";
  return toks[n];
}
